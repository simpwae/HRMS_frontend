...
export default function SelfService() {
  ...
  const scopeFaculty = user?.faculty;

  const scopedEmployees = useMemo(
    () => employees.filter((e) => !scopeFaculty || e.faculty === scopeFaculty),
    [employees, scopeFaculty],
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Faculty view, approvals, and attendance snapshot</p>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Faculty Staff', value: stats.staff, icon: UsersIcon, color: 'indigo' },
          {
      ...
      <Card>
        ...
        {pendingLeaves.length === 0 ? (
          <p className="text-sm text-gray-500">No pending leaves for your faculty.</p>
        ) : (
